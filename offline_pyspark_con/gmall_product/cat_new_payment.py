...
.select(
    col("putaway_date"),
    col("pay_amount"),
    year("putaway_date").alias("year"),  # 提取年份
    month("putaway_date").alias("month"),  # 提取月份
    dayofmonth("putaway_date").alias("day"),  # 提取日
    date_format("putaway_date", "EEEE").alias("weekday"),  # 提取星期几（如Monday）
    date_format("putaway_date", "MM-dd").alias("month_day")  # 提取月-日格式（如01-01）
)

# 按日期聚合支付金额（用于热力图权重）
daily_summary = tmall_products.groupBy(
    "year", "month", "day", "month_day", "weekday"  # 按日期维度分组
).agg(
